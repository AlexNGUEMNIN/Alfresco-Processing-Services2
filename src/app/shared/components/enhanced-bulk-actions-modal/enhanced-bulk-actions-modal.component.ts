...
export class EnhancedBulkActionsModalComponent implements OnInit, OnChanges {
  ...
  availableGroups: Group[] = [
    ...
    { id: '4', name:  'Risk Management', description: 'Gestion des risques', memberCount: 6 }
  ];
  ...
  getActionCount(action: string): number {
    switch (action) {
      case 'resume':
        return this.selectedItems.filter(item => item.status === 'suspende').length;
      case 'suspend':
        return this.selectedItems.filter(item => item.status === 'in-progress').length;
      case 'stop':
        return this.selectedItems.filter(item => 
          item.status === 'in-progress' || item.status === 'suspended' || item.status === 'pending'
        ).length;
      case 'transfer':
  ...
  // ===== PRIORITY METHODS =====
  setNewP riority(priority: 'high' | 'medium' | 'low'): void {
  ...
  setQuickDate(value: string): void {
    ...
    // Set to end of day
    targetDate.setH ours(23, 59, 59, 999);
  ...
  getActionSummary(): string {
    ...
    switch (this.selectedAction) {
      case 'resume':
        return `Reprendre ${count} ${itemType}${count > 1 ? 's' : ''} suspendue${count > 1 ? 's' : ''}`;
      case 'suspend':
        return `Suspendre ${count} ${itemType}${count > 1 ? 's' : ''} en cours`;
      case 'stop':
        return `Arrêter définitivement ${count} ${itemType}${count > 1 ? 's' : ''}`;
      case 'transfer':
        let target = '';
        if (this.transferType === 'user' && this.selectedUserId) {
          ...
          target = 'le manager respons able';
        }
        return `Transférer ${count} ${itemType}${count > 1 ? 's' : ''} vers ${target}`;
      case 'change-priority':
